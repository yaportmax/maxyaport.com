...
export const mapBlockType = defineType({
  ...
  fields: [
    ...
    defineField({
      name: 'globe',
      title: 'Use globe projection',
      description: 'Best for big travel/driving routes. Short trail segments usually read better with this off.',
      type: 'boolean',
      initialValue: false,
    }),
    defineField({
      name: 'followCamera',
      title: 'Follow marker while scrolling',
      description: 'Moves the map camera with the animated route marker.',
      type: 'boolean',
      initialValue: false,
    }),
    defineField({
      name: 'pitch',
      title: 'Follow camera pitch',
      type: 'number',
      initialValue: 48,
      hidden: ({parent}) => !parent?.followCamera,
      validation: (rule) => rule.min(0).max(70),
    }),
    defineField({
      name: 'bearing',
      title: 'Follow camera bearing',
      type: 'number',
      initialValue: 0,
      hidden: ({parent}) => !parent?.followCamera,
